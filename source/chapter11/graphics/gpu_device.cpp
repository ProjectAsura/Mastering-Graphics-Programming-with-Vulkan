...
PipelineHandle GpuDevice::create_pipeline( const PipelineCreation& creation, const char* cache_path ) {
    ...
    for ( u32 l = 0; l < num_active_layouts; ++l ) {
        ...
        rprint( "Layout debug for pipeline %s\n", creation.name );
    ...
    if ( shader_state_data->graphics_pipeline ) {
        ...
        temporary_allocator->free_marker( current_marker );
    } else {
...
//
//
void GpuDevice::fill_write_descriptor_sets( GpuDevice& gpu, const DescriptorSetLayout* descriptor_set_layout, DescriptorSet* descriptor_set,
    ...
    for ( u32 r = 0; r < num_resources; r++ ) {
        ...
        if ( binding_data_index >= descriptor_set_layout->num_bindings ) {
            rprint( "Error adding binding %u, layout has max %u bindings\n", binding_data_index, descriptor_set_layout->num_bindings );
            continue;
        }
...
RenderPassHandle GpuDevice::create_render_pass( const RenderPassCreation& creation ) {
    ...
    if ( !dynamic_rendering_extension_present ) {
        ...
        RASSERT( creation.shading_rate_image_index == k_invalid_index );
...
FramebufferHandle GpuDevice::create_framebuffer( const FramebufferCreation& creation ) {
    ...
    if ( !dynamic_rendering_extension_present ) {
        ...
        RASSERT( creation.shading_rate_attachment.index == k_invalid_index );
...
void GpuDevice::create_swapchain() {
    ...
    if ( synchronization2_extension_present ) {
        ...
        check( vkQueueSubmit2KHR( vulkan_main_queue, 1, &submit_info, VK_NULL_HANDLE ) );
    } else {
        VkSubmitInfo submitInfo = { VK_STRUCTURE_TYPE_SUBMIT_INFO };
        submitInfo.commandBufferCount = 1;
        submitInfo.pCommandBuffers = &command_buffer->vk_command_buffer;

        check( vkQueueSubmit( vulkan_main_queue, 1, &submitInfo, VK_NULL_HANDLE ) );
    }
...
PagePoolHandle GpuDevice::allocate_texture_pool( TextureHandle texture_handle, u32 pool_size ) {
...
void GpuDevice::destroy_page_pool_instant( ResourceHandle handle ) {
...
void GpuDevice::bind_texture_pages( PagePoolHandle pool_handle, TextureHandle texture_handle, u32 x, u32 y, u32 width, u32 height, u32 layer ) {
    PagePool* page_pool = access_page_pool( pool_handle );
    if ( page_pool == nullptr ) {
        RASSERT( false );
        return;
...
void GpuDevice::present( CommandBuffer* async_compute_command_buffer ) {
    ...
    RASSERT( num_queued_command_buffers < 4 );
    ...
    if ( timeline_semaphore_extension_present ) {
        ...
        if ( synchronization2_extension_present ) {
            ...
            check( vkQueueSubmit( vulkan_main_queue, 1, &submit_info, VK_NULL_HANDLE ) );
        ...
        if ( synchronization2_extension_present ) {
            ...
            check( vkQueueSubmit2KHR( vulkan_main_queue, 1, &submit_info, render_complete_fence ) );
            ...
            check( vkQueueSubmit( vulkan_main_queue, 1, &submit_info, render_complete_fence ) );
    ...
    if ( texture_to_update_bindless.size ) {
        ...
        static constexpr u32 k_num_writes_per_frame = 24;
        ...
        for ( i32 it = texture_to_update_bindless.size - 1; it >= 0; it-- ) {
            ...
            //if ( texture_to_update.current_frame == current_frame )
            {
                ...
                // Check that necessary descriptors are available, otherwise finish for this frame.
                const u32 needed_descriptors = ( texture->flags & TextureFlags::Compute_mask ) ? 2 : 1;
                if ( current_write_index + needed_descriptors >= k_num_writes_per_frame - 1 )
                    break;
...
PagePool* GpuDevice::access_page_pool( PagePoolHandle page_pool ) {

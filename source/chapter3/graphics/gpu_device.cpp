...
static void vulkan_create_swapchain_pass( GpuDevice& gpu, const RenderPassCreation& creation, RenderPass* render_pass ) {
    ...
    util_add_image_barrier( command_buffer->vk_command_buffer, depth_texture_vk->vk_image, RESOURCE_STATE_UNDEFINED, RESOURCE_STATE_PRESENT, 0, 1, true );
    ...
    if ( texture_to_update_bindless.size ) {
        ...
        for ( i32 it = texture_to_update_bindless.size - 1; it >= 0; it-- ) {
            ...
            //if ( texture_to_update.current_frame == current_frame )
            {
                ...
                // Update image view if valid, otherwise use the dummy one for deleted textures.
                if ( texture->vk_format != VK_FORMAT_UNDEFINED ) {
